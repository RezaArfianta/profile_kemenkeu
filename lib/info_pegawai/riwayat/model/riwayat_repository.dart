...
class RiwayatRepository {
  ...
  Future<List<Pangkat>> getRiwayatPangkat(String nip) async {
    try {
      ...
      return List<Pangkat>.from(res.data['Data'].map((i) => Pangkat.fromJson(i)));
    } on DioError catch (e) {
      print("erir $e");
      return null;
    }
  ...
  Future<List<Jabatan>> getRiwayatJabatan() async {
    try {
      ...
      log(jsonEncode(res.data));
      return List<Jabatan>.from(res.data['Data'].map((i) => Jabatan.fromJson(i)));
    } on DioError catch (e) {
      print(e);
      return null;
    }
  ...
  Future<List<Diklat>> getRiwayatDiklat(String nip) async {
    try {
      Response res = await _repository.dio.get(
          '${ApiUrl.serviceUrl}/hris/diklat/Riwayat/GetByNip/$nip');
      log(jsonEncode(res.data));
